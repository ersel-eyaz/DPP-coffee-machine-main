...
| P2 | Product | `product_anomaly.json` | Deterministic and statistical/ML product findings | both | 12 frozen findings: 9 rule-based, one Z-Score, one IQR, and one Isolation Forest | verified |
| P3 | Product | `product_similarity.json` | Character-sequence matching for a field label and unit | harmonize | `modelWeigh` and `kilogrm` are accepted by fuzzy matching; the clean output reconstructs P0 | verified |
...
| E2 | Emission | `emission_anomaly.json` | Calculation, unit, scope/category, and statistical findings | both | Five frozen findings: four rule-based and one IQR | verified |
| E3 | Emission | `emission_similarity.json` | Character-sequence and semantic matching of controlled values | harmonize | `power consumptin` is resolved by fuzzy matching and the Scope paraphrase by semantic matching; the clean output reconstructs E0 | verified |
...
| S2 | Service | `service_structure.json` | Required relation, replacement identity, and part-type consistency | both | Three frozen structural findings; the two consistency errors support integrated save blocking | verified |
| S3 | Service | `service_similarity.json` | Semantic and character-sequence matching of service texts | harmonize | The diagnosis is resolved semantically and the symptom by fuzzy matching; the clean output reconstructs S0 | verified |
...
- P2: `product_height_range`, `product_profile_range_mismatch`,
...
- E2: `emission_unit_incompatibility`,
...
configuration variant of P2 rather than a separate JSON-LD scenario.
